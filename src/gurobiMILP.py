...
class gurobi_MILP:
    ...
    def set_DRO_obj_constraints(self):
        ...
        if self.is_case2:
            ...
            for j in range(self.nTimeslot):
                ...
                for i in range(self.nRES):
                    if not self.is_igdt_risk_averse:
                        self.C_list.append(self.dayahead_smp[j]*self.res_list[i].max_power)
                    else:
                        self.C_list.append((1-self.alpha)*self.dayahead_smp[j]*self.res_list[i].max_power)
                ...
                for n in range(self.nScen):
                    ...
                    for i in range(self.nRES):
                        ...
                        if not self.is_igdt_risk_averse:
                            ...
                            lhs += (1-self.alpha) * self.dayahead_smp[j] * xi_hat
                    ...
                    for i in range(self.nRES):
                        ...
                        if not self.is_igdt_risk_averse:
                            self.m.addConstr( self.YY == H1 * self.gamma_obj[i,n,j] + H2 * self.gamma_obj[i+self.nWT,n,j] - self.dayahead_smp[j]*self.res_list[i].max_power, name=f'YYe_{i}_{n}_{j}')
                        else:                            
                            self.m.addConstr( self.YY == H1 * self.gamma_obj[i,n,j] + H2 * self.gamma_obj[i+self.nWT,n,j] - (1-self.alpha)*self.dayahead_smp[j]*self.res_list[i].max_power, name=f'YYe_{i}_{n}_{j}')
                    ...
                    #self.m.addConstr( self.lambda_obj[j] >= max(self.C_list), f"Const_norm_obj_comp_{j}")     
            print("end set_dro_obj_constriants")   
        else:
    ...
    def set_DRCC_Constraint(self):       
       ...
       for t in range(self.nTimeslot):
           ...
           rhs_sc = self.theta[t] * self.nScen  
    ...
    def set_base_Objectives(self):
        ...
        if self.is_bid_DRO:
            ...
            self.obj_sum_without_bid = - self.obj2 - self.obj3
            obj = self.obj1 - self.obj2 - self.obj3
